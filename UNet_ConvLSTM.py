...
## ------------------------ Encoder * 10 + Decoder * 10 + ConvLSTM ---------------------- ##
class UNetCNN(nn.Module):
    def __init__(self, channelExponent=3, dropout=0.):
        ...
        self.layer5 = blockUNet(channels * 4, channels * 4, 'layer5', transposed=False, bn=True, relu=False,
                                dropout=dropout, size=(2, 3), pad=0)

        self.dlayer5 = blockUNet(channels * 4, channels * 4, 'dlayer5', transposed=True, bn=True, relu=True,
...
if __name__ == "__main__":
    ...
    print(output_tensor.shape)  # torch.Size([20, 10, 1, 32, 64])
    ...
    layer_output, last_states = convlstm(output_tensor)

    print(layer_output[0].size())  # (batch_size, time_step, output_channels, height, width) of the last layer
    # torch.Size([20, 10, 1, 32, 64])  or torch.Size([20, 1, 1, 32, 64])  depends on only_last_time=True?
